...
def rowandvalues(row):
	for i in row:
		if not row[i]:
			row[i] = 0  # ENHANCE: handle the null vals
	...
	with conn.cursor() as cursor:
		try:
			cursor.execute(f"""
				...
				DROP TABLE IF EXISTS unlogged_{TableName};
				CREATE UNLOGGED TABLE unlogged_{TableName} (
					CensusTract         NUMERIC,
					State               TEXT,
					County              TEXT,
					TotalPop            INTEGER,
...
def adding_constraints(conn):
	with conn.cursor() as cursor:
		cursor.execute(f"""
		INSERT INTO {TableName} SELECT * from unlogged_{TableName};
		DROP TABLE unlogged_{TableName};
