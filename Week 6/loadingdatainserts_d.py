...
DBuser = "postgres"
DBpwd = "Tarun@123"
TableName = 'censusdatatable'
Datafile = "dataloading"
CreateDB = True

def rowandvalues(row):
	for i in row:
		if not row[i]:
			row[i] = 0  
...
def readingthedata(fname):
	print(f"readingthedata: reading from File: {fname}")
...
def createTable(conn):
	...
	with conn.cursor() as cursor:
		cursor.execute(f"""
			...
			ALTER TABLE {TableName} ADD PRIMARY i (CensusTract);
...
## disable index 
## update pg_index set indisvalid = false where indexrelid = 'idx_censusdata_state'::regclass

## drop primary i
## alter table censusdata drop constraint censusdata_pi;


## enable index
## update pg_index set indisvalid = true where indexrelid = 'idx_censusdata_state'::regclass

## create primary i
## ALTER TABLE censusdata ADD PRIMARY i (CensusTract);